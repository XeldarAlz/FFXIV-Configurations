...
float4 DrawMotionVectors(float2 uv) {
    ...
    float2 motion_res = float2(BUFFER_WIDTH, BUFFER_HEIGHT) / 8.0;
    ...
    float2 motion = tex2D(sCoarseFlowL0_B, motion_grid).xy;
...
float2 ComputeFlow(sampler2D source_flow_sampler, float2 uv, int mip1, int mip2)
{
    ...
    // The final motion vector is prediction + residual. Searching for residual is more stable numerically
    ...
    // 1. Get the costs of the central pixel and its horizontal/vertical neighbors.
...
float2 PS_FinalFlow(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
...
float2 PS_GlobalFlow(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    ...
    for(int i = 0; i < 32; i++)
    {
        ...
        if(depth < 0.999)
        {
            float2 flow = tex2Dlod(sCoarseFlowL0_B, float4(SPARSE_SCREEN[i], 0, 0)).xy;
...
{
    return tex2D(sCoarseFlowL0_B, uv).xy;
}
...
{
    return BilateralFilter(sCoarseFlowL3_A, uv, rcp(tex2Dsize(sCoarseFlowL3_A, 0)), 6);
}

float2 PS_SpatialFilterL2(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    return BilateralFilter(sCoarseFlowL2_A, uv, rcp(tex2Dsize(sCoarseFlowL2_A, 0)), 5);
}

float2 PS_SpatialFilterL1(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    return BilateralFilter(sCoarseFlowL1_A, uv, rcp(tex2Dsize(sCoarseFlowL1_A, 0)), 4);
}

float2 PS_SpatialFilterL0(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    return BilateralFilter(sCoarseFlowL0_A, uv, rcp(tex2Dsize(sCoarseFlowL0_A, 0)), 3);
}

float2 PS_SpatialFilterFinal(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    return BilateralFilter(sCoarseFlowL0_A, uv, rcp(tex2Dsize(sCoarseFlowL0_A, 0)), 3);
}
...
float PS_ComputeConfidence(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    ...
    float2 flow = tex2D(sCoarseFlowL0_B, uv).xy;
    ...
    float curr_luma = tex2Dlod(sCurrLuma, float4(uv, 0, 3)).r;
    float prev_luma = tex2Dlod(sPrevLuma, float4(prev_uv, 0, 3)).r;
    ...
    float2 destination_flow = tex2D(sCoarseFlowL0_B, prev_uv).xy;
